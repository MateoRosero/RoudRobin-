...
public class App extends JFrame{
    ...
    private void insertarDtos(){
        if (textFieldId.getText().matches("[a-zA-Z0-9]+") && !textFieldId.getText().isBlank() ){
            ...
            if (textFieldCedula.getText().matches("[0-9]*") && !textFieldCedula.getText().isBlank()){
    ...
    private void eliminarHIstorial(){
        if (!r.getHistorialx().isEmpty()){
            r.getHistorialx().clear();
    ...
    private void ultimoElementoStack(){
        if (!r.getHistorialx().isEmpty()){
            textAreaHistorial.setText("Id: " + r.getHistorialx().peek().getIdProceso() + "\nCedula: " + r.getHistorialx().peek().getCedula() + "\n\n");
        }else{
            textAreaHistorial.setText("Aun no existe un historial");
        }
    }
    ...
    private void mostrarHistorial(){
        if (!r.getHistorialx().isEmpty()){
            textAreaHistorial.setText("");
            ListIterator<Proceso> iterador = r.getHistorialx().listIterator(r.getHistorialx().size());
